...
- **MCP server** (`src/mcp-cli.ts` → `src/mcp/server.ts`): 独立进程，通过 stdio 与 Claude Code 通信，提供 `get_annotations`、`get_file_status`、`open_file` 三个 tools
...
- **降级模式**: TUI 未运行时，`get_annotations` 直接读 `.markcli.json` 文件
