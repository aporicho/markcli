...
3. **批注持久化**: 批注存为 `<file>.markcli.json`（同目录），使用 W3C Text Anchor（quote + prefix + suffix）定位，`textAnchor.ts` 的 `relocateAnchor()` 在文件变更后通过精确搜索 → 模糊匹配（approx-string-match）重新定位
